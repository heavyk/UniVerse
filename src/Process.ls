...
class Blueprint extends Fsm
	(@refs, opts) ->
		...
		if typeof refs isnt \object
			...
			throw new Error "you have to reference a PoetryBook for a blueprint because we save the imbuement into the poetry narrator, obviously"
	...
	imbue: (narrator) ->
		...
		# lala = @refs.narrator._[@encantador][@version][id] = new blueprint_inst @, key # {lala:1234}
			# debugger
				...
				req = Http.get {
					...
					res.on \end ~>
						...
						if res.statusCode is 200
							...
							@_blueprint = ToolShed.objectify data, {require: @refs.narrator.refs.require}, {name: @namespace}
