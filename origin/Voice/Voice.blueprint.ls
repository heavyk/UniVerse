...
encantador: \Voice
incantation: \Voice # this and the following are the path
type: \Cardinal
...
machina:
	states:
		uninitialized:
			onenter: ->
				# if @model.state isnt \ready
				# 	@model.once \ready ~> @transition \load
				# else @transition \load
				poetry = @poetry
				# get the word
				word = poetry.Word[@encantar].inst null
				bp = word.bp._blueprint.layout
				# get the word's layout
