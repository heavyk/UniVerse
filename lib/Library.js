...
var Http, assert, DeepDiff, ref$, Fsm, ToolShed, DaFunk, _, UniVerse, Blueprint, StoryBook, Library, out$ = typeof exports != 'undefined' && exports || this;
Http = require('http');
assert = require('assert');
DeepDiff = require('deep-diff');
ref$ = require('MachineShop'), Fsm = ref$.Fsm, ToolShed = ref$.ToolShed, DaFunk = ref$.DaFunk, _ = ref$._;
...
out$.Library = Library = (function(superclass){
  ...
  prototype.states = {
    uninitialized: {
      ...
      'browser:onenter': function(){
        var Dnode, Shoe, stream, d, this$ = this;
        Dnode = require('dnode');
        Shoe = require('shoe');
        stream = Shoe('/dnode');
        d = Dnode({
          Blueprint: function(cmd, path, dd){
            var bp, diff;
            switch (cmd) {
            case 'diff':
              if (bp = this$.blueprints[dd._key]) {
                diff = dd.diff;
                ToolShed.set_obj_path(diff.path, bp._blueprint, diff.rhs);
                return bp.emit('diff', diff);
              }
            }
          }
        });
    ...
    ready: {
      ...
      fetch: function(fqvn, book, cb){
        var incantation, i, encantador, version, long_incantation, bp, refs, this$ = this;
        ...
        if (bp = this.blueprints[long_incantation]) {
          ...
          return bp.imbue(book, function(err, _constructor, bp){
            if (err) {
              this$.debug.todo("make the blueprint you referenced ... " + long_incantation);
            } else if (typeof cb === 'function') {
              this$.debug("BLUEPRINT IMBUED");
            }
            return cb(err, bp);
          });
        }
