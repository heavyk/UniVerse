...
Tone = {
  ...
  eventListeners: {
    valid: function(path, v){
      var p;
      if (p = this._parts[path]) {
        $(p).removeClass('has-error').addClass('has-success');
      }
      this._form_fields[path] = true;
    },
    invalid: function(path, v){
      var p;
      if (p = this._parts[path]) {
        $(p).removeClass('has-success').addClass('has-error');
      }
      this._form_fields[path] = false;
    }
  },
  cmds: {
    verify: function(){
    ...
    field_entry: function(part, sv){
      var voice, _part, E, changed_val, type, el, icons, iin, this$ = this;
      voice = this;
      ...
      changed_val = function(e){
        var v, val;
        v = typeof sv.onchange === 'function' && typeof (val = sv.onchange.call(voice, voice._xp, e)) !== 'undefined'
          ? val
          : e.target.value;
        if (v) {
          return voice.set(part, v);
        }
      };
      ...
      if (sv.required) {
        this._required_parts.push(part);
      }
      E.rC(_part, el);
      return el;
    }
...
Meaning = (function(superclass){
  ...
  prototype.get = function(path, no_default){
    ...
    if (typeof (v = ~path.indexOf('.')
      ? ToolShed.obj_get_path(this._xp, path)
  ...
  prototype.set = function(path, val){
    ...
    prev_val = ~path.indexOf('.')
      ? ToolShed.obj_get_path(this._xp, path, val)
      : this._xp[path];
    if (!_.isEqual(val, prev_val)) {
      ...
      this.validate(path, function(err){
        if (err) {
          return console.log("INVALID", err);
        } else {
  ...
  prototype.validate = function(path, cb){
    var __validate, _validate, i$, ref$, len$, v, k, lv, this$ = this, results$ = [];
    __validate = function(v, def, cb){
      var def_type, tv, fn, ret;
      def_type = (function(){
        switch (def.type) {
        case 'key':
          return 'string';
      ...
      if (typeof (fn = def.validate) === 'function' && !(ret = fn.call(this$, v, path))) {
        return cb.call(this$, {
          code: 'ENOTVALID'
        });
      } else if (typeof (fn = def.validateAsync) === 'function') {
        throw new Error("validateAsync not yet supported");
        return ret = fn.call(this$, v, path, function(err, new_v){
          var v;
          if (typeof new_v !== 'undefined') {
            v = new_v;
            this.set(path, new_v);
          }
          return cb.call(this, err, v);
        });
      } else {
        return cb.call(this$, null, v);
      }
    };
    _validate = function(v, def, k, cb){
      return __validate(v, def, function(err, vv){
        var idx;
        if (err) {
          if (lv.required) {
            if (!~this$._required_parts.indexOf(k)) {
              this$._required_parts.push(k);
            }
          }
          this$.emit('invalid', k, err);
        } else {
          if (~(idx = this$._required_parts.indexOf(k))) {
            this$._required_parts.splice(idx, 1);
          }
          this$.emit('valid', k, vv);
        }
        if (this$._required_parts.length === 0) {
          this$.emit('all-good');
        } else {
          this$.emit('still-need', this$._required_parts);
        }
    ...
    if (typeof path === 'function') {
      cb = path;
      for (i$ = 0, len$ = (ref$ = Object.keys(this._dirty_vals)).length; i$ < len$; ++i$) {
        v = i$;
        k = ref$[i$];
        if (lv = this._bp._blueprint.layout[k]) {
          results$.push(_validate(v, lv, k, cb));
        }
      }
      return results$;
    } else {
      if (lv = ToolShed.obj_get_path(this._bp._blueprint.layout, path)) {
        v = ToolShed.obj_get_path(this._dirty_vals, path);
        return _validate(v, lv, path, cb);
