...
ExperienceDB = (function(superclass){
  ...
  prototype.states = {
    uninitialized: {
      onenter: function(){
        return this.debug("we're gonna connect now...");
      }
    },
    connected: {
      onenter: function(){
        return this.debug("connected to the database now..");
      }
    },
    disconnected: {
      onenter: function(){
        return this.debug("disconnected from the universe");
      }
    }
  };
  ...
  prototype._req = function(method, key, vals, cb){
    ...
    if (key) {
      if (assert && method === 'delete') {
        if (~this.deleted.indexOf(key)) {
          this.debug("this shouldn't happen... kinda an insurance policy while testing");
          debugger;
        } else {
      ...
      if (this.__request[key]) {
        this.debug("already saving... throttle this");
        this.debug("TODO: add 'saving' 'saved' 'save_timeout' events plus debounce with a cooldown to try again");
    ...
    req = Http.request(opts, function(res){
      ...
      res.on('end', function(){
        var xp, code;
        this$.debug("done with the request:", method, id);
        this$.__request[key] = null;
        if (res.statusCode === 200) {
          if (method !== 'delete') {
            xp = DaFunk.objectify(data, {}, {
              name: id
            });
            if (xp._key && !xp._id) {
...
Quest = (function(superclass){
  ...
  prototype._inquire = function(inquiry, depth){
    ...
    while (ii && (i = ii.shift())) {
      ...
      case 'FILTER':
        if (ii[0].charAt(0) === '(') {
          this.debug("quickly parse the parens using a loop, then parse the expression into a function");
          joined = ii.join(' ');
          this.debug("this expression: '%s'", joined);
    ...
    this.book.memory[vi].on('found', function(key, xp){
      this$.debug("we found xp", xp, this$.keys.indexOf(key));
      ...
      request: function(opts){
        ...
        this.__loading = req = Http.request({
          ...
          return res.on('end', function(){
            ...
            this$.debug("this is the result of loading a quest: (" + this$.key + "):", res.statusCode);
            this$._loading = null;
            if (!opts.cursor) {
              this$.emit('empty');
            }
