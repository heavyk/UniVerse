	...
	imbue: (book) ->
		...
		# else if not (blueprint_inst = library.blueprints[@encantador])
			...
			if @encantador isnt @incantation
				# console.debug "imbue" "encantador", @encantador, "incantation", @incantation
				eval """
				(function(){
			...
			# lala = DaFunk.extend element::, @_blueprint.machina
			...
			onenter: ->
				process_bp = ~>
					...
					task.end (err, res) ~>
						...
						console.debug "initialized blueprint", @fqvn
						# debugger
				...
				req = Http.get {
					...
					res.on \end ~>
						# console.log "done with the request:", res
						if res.statusCode is 200
							console.debug "gonna create a blueprint...", @encantador, @incantation
